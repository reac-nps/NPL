...
def compute_distance_matrix(feature_vector_key, training_set):
    N = len(training_set)
    ...
    for i in range(N):
        if i % 100 == 0:
            print(i)
        for j in range(i, N):
            distance_vector = training_set[i].get_feature_vector(feature_vector_key) - training_set[j].get_feature_vector(feature_vector_key)
...
def maximum_distance_sampling(original_set, n_structures, feature_key):
    dist_matrix = compute_distance_matrix(feature_key, original_set)
